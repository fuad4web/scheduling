...
<html>
  ...
  <body class="hold-transition skin-yellow layout-top-nav" onload="myFunction()">
    ...
    <?php 
        ...
        $query=mysqli_query($con,"SELECT b.subject_code, b.subject_title, a.rank, a.rank_id, b.subject_id FROM subject AS b INNER JOIN rank AS a ON b.rank_id = a.rank_id ORDER BY subject_code")or die(mysqli_error());
          //using of join too here
    ...
    ?>
                ...
                <?php 
                  ...
                  // }
                ?>
                  <a id="click" href="subject.php?id=<?php echo $id;?>&code=<?php echo $code;?>&title=<?=$title?>">
                    <i class="glyphicon glyphicon-edit text-blue"></i></a>
            ...
            <div class="col-md-3">
              <div class="box box-warning">
                ...
                <select class="form-control select2" name="dept" required>
                  ...
                  <?php 
                  $query2=mysqli_query($con,"SELECT b.dept_code, b.dept_name, a.rank, a.rank_id, b.dept_id FROM rank AS a INNER JOIN dept AS b ON a.rank_id = b.rank_id ORDER BY dept_id")or die(mysqli_error($con));
                  
                    while($row=mysqli_fetch_array($query2)){
                  ?>
                  <option value=""></option>
                    <option value="<?php echo $row['rank_id'];?>"><?php echo $row['dept_name'];?></option>
                  <?php }
                ...
                <input type="hidden" class="form-control" id="id" name="id" value="<?php echo @$_REQUEST['id'];?>">
								<input type="text" class="form-control" id="code" name="code" value="<?php echo @$_REQUEST['code'];?>" placeholder="Course Code">
								<input type="text" class="form-control" id="class" name="title" value="<?php echo @$_REQUEST['title'];?>" placeholder="Course Title" required>
						  </div><!-- /.form group -->
					</div>
				  </div>	
